...
/**
 * Provides a 'Programs Type' ExternalDataSource.
 *
 * @ExternalDataSource(
 *   id = "programs_type",
 *   name = @Translation("Programs Type"),
 *   description = @Translation("This Plugin will gather a list of Howard Programs Types.")
 * )
 */
class ProgramsType extends ExternalDataSourceBase {
  ...
  public function getPluginDefinition() {
    return $this->t('This Plugin will gather a list of Howard Programs Types.');
  }
  ...
  public function getResponse() {
    $cid = 'hp_programs_feed_external_data_source_programs_type';
    ...
    if ($cache = \Drupal::cache()->get($cid)) {
      ...
      $client = new Client();
      try {
        // Program_type is the Types endpoint on howard programs.
        $response = $client->get('https://programs.howard.edu/api/program_type/');
        $data = json_decode($response->getBody()->getContents());
        $data = $data->data;
      }
      catch (GuzzleException $e) {
        \Drupal::logger('external_data_source')->error('HTTP request failed for Programs Types API: @message', ['@message' => $e->getMessage()]);
      }
